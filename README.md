...
### `filemagic.sh` - Fetch and process documentation content
...
Builds the `file_processing.sif` Apptainer container (if not already present), then runs two content-fetching steps inside it:

1. **`file_magic.py`** — clones GitHub repos and converts their docs into flat `.md` files under `docs/`.
2. **`scrape_srcc.py`** — crawls `https://srcc.stanford.edu` and writes scraped content as `.md` files to `docs/srcc/`.

**`scrape_srcc.py` strategy:**
- **JSON:API pass** — fetches structured content types (`stanford_news`, `stanford_person`, `stanford_policy`, `stanford_publication`, `stanford_course`) via Drupal's JSON:API endpoint, converting each node to a Markdown file with YAML front matter.
- **HTML crawl pass** — crawls remaining pages (especially `stanford_page`, which uses Layout Builder) via standard HTTP, extracting content from Layout Builder regions and falling back to common content selectors.
- Skips noise paths (`/user`, `/admin`, `/search`, etc.) and non-HTML assets.

**Environment variables for `scrape_srcc.py`:**

| Variable | Default | Description |
|---|---|---|
| `SRCC_OUTPUT_DIR` | `docs/srcc` | Output directory for scraped SRCC content |
| `SRCC_MAX_PAGES` | `500` | Maximum pages to crawl in the HTML pass |
| `LOG_FILE` | `magicFile.log` | Log file path |

**Output:**
- `docs/` — cloned repo documentation (from `file_magic.py`)
- `docs/srcc/` — scraped SRCC website content (from `scrape_srcc.py`), one `.md` file per page
...
| `SRCC_OUTPUT_DIR` | *(scrape_srcc.py only)* | Output directory for scraped SRCC content (default: `docs/srcc`) |
| `SRCC_MAX_PAGES` | *(scrape_srcc.py only)* | Maximum HTML pages to crawl from srcc.stanford.edu (default: `500`) |
...
├── stop_multi_gpu.sh        # Stop all services
├── filemagic.sh             # Fetch & process docs content (GitHub repos + SRCC site)
├── chatbot.def             # Container definition (chatbot)
├── chatbot.sif             # Built container (generated)
├── file_processing.def     # Container definition (doc processing)
├── file_processing.sif     # Built container (generated)
├── config.yaml             # Configuration
├── requirements.txt        # Python dependencies
├── file_magic.py           # Clones GitHub repos, converts docs to .md
├── scrape_srcc.py          # Scrapes srcc.stanford.edu, outputs to docs/srcc/
├── app/
...
├── docs/                   # Processed documentation (generated by filemagic.sh)
│   └── srcc/              # Scraped SRCC website content
